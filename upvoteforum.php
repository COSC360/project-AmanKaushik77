...
<?php
    ...
    $sql3 = "SELECT * FROM downvotes where user_id = ? AND post_id = ?";
    ...
    elseif($row2 = $statement2->fetch()){
        $sql4 = $pdo->prepare("DELETE FROM downvotes WHERE post_id=? AND user_id = ?");
        $sql4->execute([$pid,$uid]);

        $sql2 = $pdo->prepare("INSERT INTO upvotes (post_id, user_id) VALUES (?, ?)");
